...
export class FacialAnimator{
  ...
  update(dt,target){
    ...
    const fatigue=clamp((100-Math.min(f.physical,f.consciousness))/100),attack=f.state==='Attack',block=f.state==='Block',stun=f.state==='Stun'||this.hitReact>0,danger=f.danger?.()??false,victory=f.state==='Victory',special=attack&&f.attackType==='special',eyeY=f.def.id==='gigachad'?.052:.045,eyeX=f.def.id==='gigachad'?.095:.088;
    ...
    r.lids?.forEach((l,i)=>{l.scale.y=.82+blinkAmt*3.1+squint*.9;l.position.y=eyeY+.029-blinkAmt*.020-squint*.004+(stun?Math.sin(this.t*25+i)*.0025:0)});
    let browOffset=-fatigue*.018+(attack?.012:0)+(stun?.020:0)+(victory?-.006:0);if(danger)browOffset+=.010;if(special)browOffset+=.010;
    r.brows?.forEach((b,i)=>{const sx=i===0?-1:1;b.position.y=.105+browOffset+(stun?Math.sin(this.t*28+i)*.004:0);b.rotation.z=sx*(special?.14:attack?.09:danger?.055:victory?.02:.035)+(stun?sx*.05:0)});
