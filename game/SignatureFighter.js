...
export class SignatureFighter extends EliteFighter{
  constructor(def,slot){super(def,slot);this.specialIndex=-1;this.metrics=freshMetrics();this.facial=new FacialAnimator(this);this.hitReaction=new HitReaction(this);this.finisherReaction=0;this.finisherStyle=0;this.finisherSource='';this.introPose=-1;}
  ...
  entrancePose(){
    if(this.introPose<0)return;const p=this.introPose,b=bell(p),id=this.def.id,side=this.slot%2?1:-1;
    this.torso.rotation.y+=side*(1-p)*.28;this.head.rotation.y-=side*(1-p)*.14;
  ...
  animate(dt,input){
    ...
    this.entrancePose();this.victoryPose();this.hitReaction?.apply();this.finisherVictimPose();
  }
