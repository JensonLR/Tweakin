...
export class Game {
    ...
    loop = () => {
        ...
        if (!this.pause && this.match) {
            ...
            this.match.tickCinematic(dt);this.presentation?.update(dt);this.updateCamera(dt);this.renderer.render(this.match.scene, this.camera);this.hooks.onHud?.(this.match);
        } else if(!this.match) {
            this.camera.fov += (47-this.camera.fov)*(1-Math.exp(-dt*4));this.camera.updateProjectionMatrix();this.menuBackdrop?.update(dt,this.camera);this.renderer.toneMappingExposure=1.02;this.renderer.render(this.menuBackdrop.scene,this.camera);
        }
    ...
    resize() {const w = innerWidth, h = innerHeight;this.camera.aspect = w / h;this.camera.updateProjectionMatrix();this.renderer.setSize(w, h);this.renderer.setPixelRatio(Math.min(devicePixelRatio, this.mobile ? 1.25 : this.settings.quality === 'high' ? 2 : 1.5));}
    ...
    destroy() { this.running = false; cancelAnimationFrame(this.raf); this.stop();this.menuBackdrop?.dispose(); this.renderer.dispose(); this.renderer.domElement.remove(); }
}
