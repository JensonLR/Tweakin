...
export class CinematicCombatMatch extends CombatMatch{
  ...
  resolveAttack(ev){
    ...
    if(ev?.type==='special'&&attacker&&target&&!wasConnected&&attacker.attackConnected){
      const style=Math.max(0,attacker.specialIndex??0);target.triggerFinisherReaction?.(style,attacker.def.id);this.cinematic=[attacker,target];this.cinematicTime=Math.max(this.cinematicTime,1.55);this.cameraShake=Math.max(this.cameraShake,1.02);
    }
  }
  finish(winner){
