...
export class PresentationDirector{
  ...
  impact(pos,kind='light',blocked=false){
    if(!this.scene||!pos)return;const strong=['heavy','grapple','weapon','special','environment'].includes(kind);this.flash=Math.max(this.flash,strong?.22:.10);this.match?.arena?.react?.(blocked?.22:kind==='special'?1.2:kind==='environment'?1.05:strong?.72:.36);const color=blocked?0xb9c8d8:kind==='special'?0xffd56b:kind==='weapon'?0xff8a58:kind==='environment'?0xffb06a:0xfff0d7,burst=new THREE.Group();burst.position.copy(pos);this.scene.add(burst);const n=this.mobile?(strong?7:4):(strong?16:8);
    for(let i=0;i<n;i++){const m=new THREE.Mesh(new THREE.SphereGeometry(strong?.035:.024,5,4),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.9,depthWrite:false,blending:THREE.AdditiveBlending})),a=Math.random()*Math.PI*2,s=Math.random()*(strong?3.2:1.7)+.8;m.userData.v=new THREE.Vector3(Math.cos(a)*s,(Math.random()*.9+.2)*s,Math.sin(a)*s);m.userData.life=.22+Math.random()*.28;m.userData.spark=true;burst.add(m)}
    if(strong){const ring=new THREE.Mesh(new THREE.TorusGeometry(.16,.018,6,28),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.7,depthWrite:false,blending:THREE.AdditiveBlending}));ring.rotation.x=Math.PI/2;ring.userData.ring=true;burst.add(ring);const light=new THREE.PointLight(color,this.mobile?2.2:4.2,3.5,2);light.userData.impactLight=true;burst.add(light)}
    burst.userData.burst=true;burst.userData.age=0;burst.userData.life=strong?.58:.42;
  }
  ...
  update(dt){
    ...
    const dead=[];for(const o of this.scene.children){if(!o.userData?.burst)continue;o.userData.age+=dt;const q=Math.min(1,o.userData.age/o.userData.life);for(const m of o.children){if(m.userData.spark){m.position.addScaledVector(m.userData.v,dt);m.userData.v.y-=5.5*dt;m.material.opacity=Math.max(0,1-o.userData.age/m.userData.life)}else if(m.userData.ring){m.scale.setScalar(1+q*4.2);m.material.opacity=(1-q)*.58;m.rotation.z+=dt*1.8}else if(m.userData.impactLight){m.intensity=(1-q)*(this.mobile?2.2:4.2)}}if(q>=1)dead.push(o)}dead.forEach(o=>{this.scene.remove(o);o.traverse(x=>{x.geometry?.dispose?.();x.material?.dispose?.()})});
