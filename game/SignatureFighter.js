...
export class SignatureFighter extends EliteFighter{
  constructor(def,slot){super(def,slot);this.specialIndex=-1;this.metrics=freshMetrics();this.facial=new FacialAnimator(this);this.finisherReaction=0;this.finisherStyle=0;this.finisherSource='';}
  setState(s,move=''){
  ...
  update(dt,input,target,arena){const ev=super.update(dt,input,target,arena);this.finisherReaction=Math.max(0,this.finisherReaction-dt);this.facial?.update(dt,target);return ev}
