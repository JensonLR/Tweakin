...
export function addFaceDetail(fighter){
  const d=fighter.def,id=d.id,h=fighter.head;if(!h||id==='wojak')return;
  ...
  for(const sx of[-1,1]){
    const eye=add(h,new THREE.SphereGeometry(.054,12,8),white,[sx*eyeX,eyeY,front],[0,0,0],[1.18,.62,.48]);
    const pupil=add(h,new THREE.SphereGeometry(.025,10,7),iris,[sx*eyeX,eyeY,front+.041],[0,0,0],[.92,.78,.32]);
    add(h,new THREE.SphereGeometry(.011,8,6),dark,[sx*eyeX,eyeY,front+.059],[0,0,0],[1,.95,.3]);
    const lid=add(h,new THREE.BoxGeometry(.128,.018,.025),skin,[sx*eyeX,eyeY+.045,front+.018],[0,0,sx*.045]);lid.scale.y=.8;
  }
  ...
  for(const sx of[-1,1])add(h,new THREE.SphereGeometry(.095,9,7),skin,[sx*.145,-.035,.275],[0,0,0],[1,.62,.48]);
  const lip=add(h,new THREE.BoxGeometry(id==='gigachad'?.16:.135,.025,.022),dark,[0,-.16,.335],[0,0,0],[1,.72,1]);lip.rotation.z=id==='trump'?.025:id==='netanyahu'?-.012:0;
  ...
  if(id==='trump'){
    const brow=std(0xc89d57,.82);for(const sx of[-1,1])add(h,new THREE.BoxGeometry(.13,.026,.035),brow,[sx*.105,.14,.31],[0,0,sx*.08]);
    add(h,new THREE.BoxGeometry(.30,.028,.018),std(0xd3ab6b,.75),[.02,.31,.02],[0,0,-.12]);
  }else if(id==='netanyahu'){
    const brow=std(0x77736e,.85);for(const sx of[-1,1])add(h,new THREE.BoxGeometry(.125,.025,.032),brow,[sx*.105,.137,.31],[0,0,sx*.04]);
  }else if(id==='kirk'){
    const brow=std(0x5b3a2a,.82);for(const sx of[-1,1])add(h,new THREE.BoxGeometry(.12,.022,.03),brow,[sx*.105,.137,.31],[0,0,sx*.025]);
  }else if(id==='agarthan'){
