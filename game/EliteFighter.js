...
export class EliteFighter extends UltraFighter{
  ...
  setWeaponVisual(kind){
    this.disposeWeapon();this.heldWeapon=kind;if(!kind)return;const g=new THREE.Group();g.position.set(0,-.55,.11);g.rotation.z=.24;const add=(geo,mat,pos=[0,0,0],rot=[0,0,0])=>{const m=new THREE.Mesh(geo,mat);m.position.set(...pos);m.rotation.set(...rot);m.castShadow=true;g.add(m);return m};
    const wood=new THREE.MeshStandardMaterial({color:0x59371f,roughness:.68,metalness:.02}),steel=new THREE.MeshStandardMaterial({color:0x777d82,roughness:.32,metalness:.76}),grip=new THREE.MeshStandardMaterial({color:0x171719,roughness:.82});
    ...
    else if(kind==='broom'){add(new THREE.CylinderGeometry(.025,.03,1.15,10),wood,[0,.05,0]);add(new THREE.BoxGeometry(.46,.15,.18),new THREE.MeshStandardMaterial({color:0x8b7046,roughness:.9}),[0,-.55,0]);for(let i=-4;i<=4;i++)add(new THREE.CylinderGeometry(.006,.01,.26,5),new THREE.MeshStandardMaterial({color:0xa89569,roughness:1}),[i*.045,-.73,0],[0,0,(i-0)*.025]);}
    this.rightFore.add(g);this.weaponVisual=g;
  }
  animate(dt,input){super.animate(dt,input);if(this.evadeTime>0){const p=this.evadeTime/.26;this.torso.rotation.z+=(this.slot?-.18:.18)*Math.sin(p*Math.PI);this.head.rotation.z-=this.torso.rotation.z*.35}if(this.parryWindow>0){this.leftFore.rotation.z-=.22;this.rightFore.rotation.z+=.22}if(this.weaponVisual&&this.state==='Attack'){const p=Math.min(1,this.stateTime/(this.attackType==='heavy'?.62:.34));this.weaponVisual.rotation.x=-Math.sin(p*Math.PI)*.24}}
}
