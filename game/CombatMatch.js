...
export class CombatMatch {
  ...
  update(dt,humanInputs=[]){
    if(this.finished)return;if(this.hitStop>0){this.hitStop=Math.max(0,this.hitStop-dt);return}
    this.frame++;this.elapsed+=dt;this.remaining=Math.max(0,this.remaining-dt);this.cameraShake=Math.max(0,this.cameraShake-dt*2.8);this.messageCooldown=Math.max(0,this.messageCooldown-dt);this.arena.update(dt,this.elapsed);
    const attackEvents=[];
    for(let i=0;i<this.fighters.length;i++){
      const f=this.fighters[i],target=this.closestTarget(f);const input=this.ais[i]?this.ais[i].update(dt,f,this.fighters,this.arena):(humanInputs[i]??{...EMPTY_INPUT});
      if(input.pickup&&!f.heldWeapon){const w=this.arena.nearestWeapon(f.group.position.x,f.group.position.z);if(w){f.setWeaponVisual(this.arena.takeWeapon(w));this.audio.pickup?.();this.emit(`${f.def.shortName} PICKS UP ${f.heldWeapon?.toUpperCase()}`)}}
      const ev=f.update(dt,input,target,this.arena);if(ev)attackEvents.push(ev)
    }
  ...
  resolveHazards(){
    const hazard=this.arena.def.hazard;if(hazard==='none')return;const s=this.arena.def.size*.48;
    for(const f of this.fighters){if(f.ko)continue;const edge=Math.max(Math.abs(f.group.position.x),Math.abs(f.group.position.z));if(edge<s*.92)continue;
      if((hazard==='ringout'||hazard==='glass')&&edge>s*.975){f.physical=0;f.consciousness=0;f.ko=true;f.setState('KO');this.emit(hazard==='glass'?'WINDOW BREAK':'RING OUT')}
      if(hazard==='subway'&&Math.abs(f.group.position.z)>s*.94&&Math.abs(f.group.position.x)<s*.28){f.receiveHit(42,'environment',this.fighters.find(x=>x!==f)??f,0,-Math.sign(f.group.position.z));this.emit('TRACK IMPACT')}
      if(hazard==='fire'&&edge>s*.94){f.physical=Math.max(0,f.physical-.08);f.consciousness=Math.max(0,f.consciousness-.12)}
    }
  }
  checkEnd(){
    const alive=this.fighters.filter(f=>!f.ko);if(alive.length<=1&&this.fighters.length>1){this.finish(alive[0]??null);return}if(this.remaining<=0){const sorted=[...this.fighters].sort((a,b)=>(b.physical+b.consciousness)-(a.physical+a.consciousness));this.finish(sorted[0]??null)}
  }
  ...
  getCameraTarget(){
    const live=this.fighters.filter(f=>!f.ko);const list=live.length?live:this.fighters;let x=0,z=0,y=1.25;for(const f of list){x+=f.group.position.x;z+=f.group.position.z}x/=Math.max(1,list.length);z/=Math.max(1,list.length);
    let spread=2.2;for(const a of list)for(const b of list)spread=Math.max(spread,Math.hypot(a.group.position.x-b.group.position.x,a.group.position.z-b.group.position.z));return{x,z,y,distance:clamp(5.1+spread*.58,5.2,9.5),cinematic:this.cinematic}
  }
  snapshots(){return this.fighters.map(f=>f.snapshot())}
  applySnapshots(snaps){for(let i=0;i<Math.min(snaps.length,this.fighters.length);i++)this.fighters[i].applySnapshot(snaps[i])}
