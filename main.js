...
async function boot() {
    ...
    if (!host || !root)
        throw new Error('TWEAKIN boot targets missing');
    ...
    const settings = isTouch ? { ...saved, quality: 'low', cameraShake: Math.min(saved.cameraShake, .5) } : saved;
