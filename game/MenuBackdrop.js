...
export class MenuBackdrop{
  constructor(mobile=false){
    ...
    const floorMat=new THREE.MeshStandardMaterial({color:0x0a0a0c,roughness:.82,metalness:.08});const floor=new THREE.Mesh(new THREE.PlaneGeometry(26,20),floorMat);floor.rotation.x=-Math.PI/2;floor.receiveShadow=true;this.scene.add(floor);
  ...
  focusFighter(id){if(!id){this.clearFocus();return}if(this.featuredId===id&&this.featured)return;this.disposeFighter(this.featured);this.featured=new SignatureFighter(getFighterDef(id),7);this.featuredId=id;this.mode='roster';const x=this.mobile?.45:2.35;this.featured.group.position.set(x,0,-.5);this.featured.group.scale.multiplyScalar(this.mobile?1.12:1.32);this.featured.group.rotation.y=-.16;this.featured.yaw=-.16;this.featured.state='Neutral';this.scene.add(this.featured.group);this.group.visible=false;this.key.target.position.set(x,1.45,-.45);this.warm.target.position.set(x,1.45,-.6)}
