...
export class PresentationDirector{
  ...
  impact(pos,kind='light',blocked=false){
    if(!this.scene||!pos)return;const strong=['heavy','grapple','weapon','special','environment'].includes(kind);this.flash=Math.max(this.flash,strong?.22:.10);this.match?.arena?.react?.(blocked?.22:kind==='special'?1.2:kind==='environment'?1.05:strong?.72:.36);const color=blocked?0xb9c8d8:kind==='special'?0xffd56b:kind==='weapon'?0xff8a58:0xfff0d7,burst=new THREE.Group();burst.position.copy(pos);this.scene.add(burst);const n=this.mobile?(strong?7:4):(strong?16:8);
