...
export class UXEnhancements{
  constructor(root,game){
    this.root=root;this.game=game;this.pending=0;this.fightKey='';this.observer=new MutationObserver(()=>this.schedule());this.observer.observe(root,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
  ...
  enhance(){this.enhanceMenu();this.enhanceBattle();this.enhanceFight();this.enhanceResult();this.enhancePause()}
  ...
  enhanceFight(){
    if(!this.root.classList.contains('fight')){this.fightKey='';return}const m=this.game?.match;if(!m?.fighters?.length)return;const key=`${m.fighters.map(f=>f.id).join('|')}|${m.arena?.def?.id}`;if(this.fightKey===key||this.root.querySelector('.versus-intro'))return;this.fightKey=key;
    const overlay=document.createElement('div');overlay.className='versus-intro';overlay.innerHTML=`<div class="versus-name left"><small>PLAYER</small><strong>${esc(m.fighters[0]?.def?.shortName)}</strong></div><div class="versus-core"><i>VS</i><span>${esc(m.arena?.def?.name)} // ${esc(m.config?.matchType)}</span></div><div class="versus-name right"><small>OPPONENT</small><strong>${esc(m.fighters[1]?.def?.shortName)}</strong></div>`;this.root.appendChild(overlay);setTimeout(()=>overlay.classList.add('exit'),900);setTimeout(()=>overlay.remove(),1380);
  }
  enhanceResult(){
